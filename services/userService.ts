...
import { UserProfile, Transaction, SystemSettings, PromoCode, BannerItem, Notification } from '../types';
...
export const processPlaceholders = (text: string, user: UserProfile): string => {
    ...
    return text
        ...
        .replace(/#lastdeposit/g, (user.lastDailyClaim || 0).toString())
        ...
        .replace(/#refercode/g, user.referralCode || "N/A");
};
...
export const publishNotification = async (title: string, content: string) => {
...
export const getSystemSettings = async (): Promise<SystemSettings> => {
  ...
  const defaults: SystemSettings = {
      ...
      minWithdraw: 100,
      maxWithdraw: 100000,
      notificationText: "Welcome to King Club, #username!",
      welcomeMessage: "Welcome to the family, #username! Get ready for the best casino experience."
  };
  return { ...defaults, ...val };
};
...
export const createUserProfile = async (uid: string, email: string, name: string, referralCodeInput?: string) => {
  ...
  const newUser: UserProfile = {
    ...
    createdAt: Date.now(), role: isAdmin ? 'admin' : 'user', hasSeenWelcome: false
  };
...
export const redeemPromoCode = async (uid: string, code: string) => {
    ...
    try {
        ...
        const promoRef = ref(db, `promo_codes/${cleanCode}`);
        const promoSnap = await get(promoRef);
        if (!promoSnap.exists()) return { success: false, error: "Invalid gift code" };
        ...
        const minReq = promo.requiredDepositAmount || 0;
        if ((user.totalDeposited || 0) < minReq) {
        ...
        const res = await runTransaction(promoRef, (p: PromoCode | null) => {
            ...
            if (!p.claimedBy) p.claimedBy = {};
            p.claimedBy[uid] = true;
            return p;
        });
        ...
        if (res.committed) {
            ...
            const processedMsg = processPlaceholders(promo.message || "Gift successfully redeemed!", user);
            return { success: true, amount: promo.amount, message: processedMsg };
        } else {
            return { success: false, error: "Redemption failed. It may have just reached its limit." };
        }
    } catch (error: any) {
...
export const initDemoData = async () => {
    ...
    if (!snap.exists()) {
        await set(ref(db, 'system_settings'), {
            ...
            welcomeMessage: "Welcome to King Club, #username! Enjoy your stay."
        });
    }
};
