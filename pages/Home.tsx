...
import { Gamepad2, Plane, Ticket, Bomb, Crown, Trophy } from 'lucide-react';
...
export const Home = () => {
  ...
  if (loading) return (
    <div className="flex flex-col h-screen items-center justify-center bg-slate-900 gap-6">
      <div className="w-28 h-28 relative">
        ...
        <img src={LOGO_URL} className="w-full h-full object-contain p-2 animate-pulse" alt="Loading Logo" />
      </div>
  ...
  return (
    <div className="pb-4 relative">
      {showWelcome && user && settings && (
          <WelcomePopup user={user} settings={settings} onClose={() => setShowWelcome(false)} />
      )}
      ...
      <div className="fixed bottom-24 right-4 z-[60] flex flex-col gap-4 items-center">
        <button 
          ...
          className="w-14 h-14 bg-white rounded-full shadow-[0_10px_25px_rgba(0,0,0,0.3)] border-2 border-yellow-500 flex items-center justify-center animate-bounce-slow active:scale-90 transition-transform duration-200 ring-4 ring-yellow-500/20"
        >
          <img src={IMG_SPIN} className="w-10 h-10 object-contain p-1" alt="Spin" />
        </button>

        <button 
          onClick={handleSupportClick}
          className="w-14 h-14 bg-white rounded-full shadow-[0_10px_25px_rgba(0,0,0,0.3)] border-2 border-gold-500 flex items-center justify-center active:scale-90 transition-transform duration-200 ring-4 ring-gold-500/20"
        >
      ...
      <header className="bg-gradient-to-r from-slate-900 to-slate-800 p-6 rounded-b-[2.5rem] shadow-2xl border-b border-gold-600/30 relative overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-6 relative z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {user?.numericId && <div className="text-[10px] text-gray-400 font-mono mt-0.5">ID: {user.numericId}</div>}
        ...
        <div className="grid grid-cols-4 gap-2 mt-8 px-1 relative z-10">
            <ActionButton label="Deposit" icon={IMG_DEPOSIT} onClick={() => navigate('/wallet', { state: { tab: 'deposit' } })} />
            <ActionButton label="Withdraw" icon={IMG_WITHDRAW} onClick={() => navigate('/wallet', { state: { tab: 'withdraw' } })} />
            <ActionButton label="Daily" icon={IMG_DAILY} onClick={handleDailyCheckIn} />
            <ActionButton label="Leader" icon={IMG_LEADER} onClick={() => navigate('/leaderboard')} />
        </div>
      </header>

      <div onClick={handleBannerClick} className={`mx-4 mt-6 h-40 rounded-3xl shadow-xl overflow-hidden relative group bg-gray-900 border border-gray-100 ${settings?.homeBanners && settings.homeBanners[bannerIndex]?.link ? 'cursor-pointer' : ''}`}>
          {settings?.homeBanners && settings.homeBanners.length > 0 ? (
             <img src={settings.homeBanners[bannerIndex].imageUrl} className="w-full h-full object-cover transition-all duration-500" alt="Banner" />
          ) : (
      ...
      <div className="p-4 mt-2 mb-20">
        ...
        <div className="grid grid-cols-2 gap-4">
          <GameCard title="Mines" image="https://uploads.onecompiler.io/43yf4q9cp/4479yhnbk/1000132485.jpg" color="from-yellow-600 to-yellow-900" icon={<Bomb />} onClick={() => navigate('/game/mines')} />
          <GameCard title="Aviator" image="https://uploads.onecompiler.io/43yf4q9cp/4479hms35/download.jpeg" color="from-red-600 to-red-900" icon={<Plane />} onClick={handleAviatorClick} />
          <GameCard title="Win Go" image="https://uploads.onecompiler.io/43yf4q9cp/4479hms35/f351f075730277cb8af52d230428b249.jpg" color="from-green-600 to-green-900" icon={<Ticket />} onClick={() => navigate('/game/wingo')} />
          <GameCard title="Roulette" image="https://uploads.onecompiler.io/43yf4q9cp/4479hms35/classic-roulette.webp" color="from-blue-600 to-blue-900" icon={<div className="w-4 h-4 rounded-full border-2 border-white"/>} onClick={() => navigate('/game/roulette')} />
          <GameCard title="Dragon Tiger" image="https://uploads.onecompiler.io/43yf4q9cp/4479hms35/unnamed.webp" color="from-purple-600 to-purple-900" icon={<span className="text-lg font-bold">VS</span>} onClick={() => navigate('/game/dragontiger')} />
        </div>
      </div>
    </div>
  );
...
const GameCard = ({ title, icon, onClick, image }: any) => (
